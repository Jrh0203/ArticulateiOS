...
- (void)viewDidLoad
{
    ...
    NSError *error = nil;
  /*  dbConn = [MongoConnection connectionForServer:@"mongodb://admin:haveyouever@articulate-shard-00-00-ztzkw.mongodb.net:27017,articulate-shard-00-01-ztzkw.mongodb.net:27017,articulate-shard-00-02-ztzkw.mongodb.net:27017/ar?ssl=true&replicaSet=Articulate-shard-0&authSource=admin" error:&error];
    collection = [dbConn collectionWithName:@"recording"];*/
...
- (void)snapButtonPressed:(UIButton *)button
{
        if(!self.camera.isRecording) {
            ...
            //    UIViewController * vc = [self.storyboard instantiateViewControllerWithIdentifier:@"SpeechViewController"];
                SFSafariViewController *safariVC = [[SFSafariViewController alloc]initWithURL:[NSURL URLWithString:@"https://image.ibb.co/n3Hc36/1jlkT1q.png"] entersReaderIfAvailable:NO];
 ...
 /*           NSDictionary *metrics = @{
                                      @"_id" : @"thisIsMyID",
                                      @"speech": @"GettysbergAddress",
                                      @"wordCount": [NSNumber numberWithInt:totalWords],
                                      @"filler": [NSNumber numberWithInt:totalFillerWords],
                                      @"duration": [NSNumber numberWithInt:elapsedTime],
                                      @"data" : @{
                                              @"wordsDone" : @"2",
                                              @"script" : @"Your Mom"
                                              }
            ...
            NSError *error = nil;*/
//            [collection insertDictionary:metrics writeConcern:nil error:&error];
...
-(void)onFinalResponseReceived:(RecognitionResult*)response {
    ...
    if (!isFinalDicationMessage) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //CFTimeInterval tempStart = pastTime;
            pastTotalWords=totalWords;
            //pastTime = startTime;
            for (int i = 0; i < [response.RecognizedPhrase count]; i++) {
                ...
                [self WriteLine:[[NSString alloc] initWithFormat:(@"[%d] Confidence=%@ Text=\"%@\""),
                                 ...
                                 phrase.DisplayText]];
                [phrase.DisplayText enumerateSubstringsInRange:NSMakeRange(0, [phrase.DisplayText length]) options:NSStringEnumerationByWords | NSStringEnumerationReverse usingBlock:^(NSString *substring, NSRange subrange, NSRange enclosingRange, BOOL *stop) {
                    if ([myArray containsObject:substring]) {
                        totalFillerWords+=1;
                    }
                    totalWords++;
                    
                }];
                //pastTotalWords = totalWords-pastTotalWords;
                
                //[self WriteLine:(@"WPM: %d\n",wpm)];
                NSTimeInterval timeInterval = fabs([startTime timeIntervalSinceNow]);
                
                double diffWords = totalWords-pastTotalWords;
                double wpm =(diffWords)/(timeInterval);
                printf("TEST %f\n",wpm*60);
                printf("WORDS %d\n",totalWords);
                printf("DURATION %f\n",timeInterval);
                printf("DIFF WORD %f\n",diffWords);
